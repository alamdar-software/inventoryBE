        ...
        for (int i = 0; i < items.size(); i++) {
            ...
            if (inventory != null) {
                ...
                if (availableQuantity >= consumedQuantity) {
                    ...
                    // Create ConsumedItem and add it to the Inventory
                    ConsumedItem consumed = new ConsumedItem();
                    consumed.setLocationName(consumedItem.getLocationName());
                    consumed.setTransferDate(LocalDate.now());
                    consumed.setSn(consumedItem.getSn());
                    ...
                    // Add the ConsumedItem to the Inventory
                    if (inventory.getConsumedItem() == null) {
                        inventory.setConsumedItem(String.valueOf(consumedItem));
                    }
                    inventory.getConsumedItem();

                    inventoryRepo.save(inventory); // Update inventory with consumed items
                    return ResponseEntity.status(HttpStatus.CREATED).body(consumed);
                } else {
                    return ResponseEntity.badRequest().body("Insufficient quantity for item: " + itemName);
                }
            } else {
